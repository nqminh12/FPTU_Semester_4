...
public class certificate extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        List<Employee> em = dao.findEmployee();
        List<Employee_Certificate> emce = dao.findEmployeeCertificate();
        request.setAttribute("ce", ce);
        request.setAttribute("em", em);
        request.setAttribute("emce", emce);
    ...
    throws ServletException, IOException {
        ...
        dao.deleteEmployeeCertificate();
        String[] checkbox = request.getParameterValues("checkbox");
        
        for (String s : checkbox) {
            String[] parts = s.split(" ");
            dao.insertEmployeeCertificate(parts[0], parts[1]);
        }
        List<Employee_Certificate> emce = dao.findEmployeeCertificate();
        request.setAttribute("emce", emce);
        request.getRequestDispatcher("certificate.jsp").forward(request, response);
