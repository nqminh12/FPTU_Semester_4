 ...
 * @author thanh
 */
@WebServlet(name = "SearchByGender", urlPatterns = {"/SearchByGender"})
public class SearchByGender extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SearchByGender</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SearchByGender at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        DAO.INSTANCE.SearchStudentByGender(gender + "");
        ...
        for (int i = 0; i < genders.length; i++) {
            s += " gender = " + genders[i] + " or ";
        }
        s = s.substring(0, s.length()-3);
        sql += "select * from Student where " + s;
        DAO.INSTANCE.SearchStudentByGenders(sql);
